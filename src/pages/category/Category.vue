<template>
  <div class="category">
    ...
    <div class="select">
      ...
      ></el-input>
      <div class="btn">
        <el-button type="primary" @click="searchHandler" size="small"
          >查询</el-button
        >
      </div>
      <el-button type="primary" @click="orderFunction" size="small"
        >查询父集中仓库</el-button
      >
    </div>
    <!-- 按钮 -->
    ...
    <el-table :data="categorys" stripe style="width: 100%">
      <el-table-column align="center" prop="id" label="仓库编号" width="80">
      </el-table-column>
      <el-table-column align="center" prop="name" label="仓库名称">
      </el-table-column>
      <el-table-column align="center" prop="num" label="序号">
      </el-table-column>
      <el-table-column align="center" prop="parentId" label="所属于仓库号">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    searchHandler() {
      ...
      //   名字查询
      obj.name = this.cangKuName;
      obj.page = 0;
      obj.pageSize = 6;
      this.searchCategoryById(obj);
    },
    //分页
    changePageNum(page) {
    ...
    orderFunction() {
      this.ajax({
        methods: "GET",
        url: "http://localhost:8099/category/orderByParent",
        success: (res) => {
          // let prop = decodeURIComponent(res)
          console.log(res);
          this.orderBy(res);
        },
      });
